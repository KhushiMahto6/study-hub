...
export default function SignUpPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    const { error } = await signUp(email, password, username.toLowerCase().replace(/\s/g, '_'), fullName);
    setLoading(false);
    if (error) {
      toast.error(error);
      return;
  ...
  return (
    <div className="space-y-6">
      ...
      <Button variant="outline" className="w-full rounded-xl gap-2" onClick={() => signInWithGoogle()} type="button">
      ...
      <form onSubmit={submit} className="space-y-4">
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <Input id="fullName" required value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="Aanya Sharma" className="pl-9 rounded-xl" />
            </div>
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="username">Username</Label>
            <Input id="username" required value={username} onChange={(e) => setUsername(e.target.value)} placeholder="aanya" className="rounded-xl" />
          </div>
        </div>
        <div className="space-y-1.5">
        ...
        <div className="space-y-1.5">
          ...
          <div className="relative">
            ...
            <Input id="password" type={show ? 'text' : 'password'} required value={password} onChange={(e) => setPassword(e.target.value)} placeholder="At least 6 characters" className="pl-9 pr-9 rounded-xl" />
      ...
      <p className="text-center text-sm text-muted-foreground">
        Already have an account? <Link href="/auth/sign-in" className="text-primary font-medium hover:underline">Sign in</Link>
      </p>
    </div>
  );
}
